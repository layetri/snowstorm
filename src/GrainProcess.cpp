...
GrainProcess::GrainProcess(
  ...
  int samplerate, Buffer *input, Buffer *outputL, Buffer *outputR) {
    ...
    length = samplerate;
    buffer = new Buffer(length, "grainProcess");
    ...
    performMapping();
    setStereo(1000);
    std::cout << "grains: " << nGrains << ", len: " << grainLen << ", division: " << divisionLen << std::endl;
...
void GrainProcess::process() {
  ...
  if(activeGrains.size() > 0) {
    for(int i = 0; i < activeGrains.size(); i++) {
      // TODO: implement envelope
      //    -> switch case: linear / exponential
      smp[0] += activeGrains[i]->process(env_length, env_shape, stereo)[0];
      smp[1] += activeGrains[i]->process(env_length, env_shape, stereo)[1];
    }
...
void GrainProcess::processParameterChanges(int nSpread, int nSize, int nShape, int nSpeed) {
  // Store readouts
  spread = nSpread;
  size = nSize;
  shape = nShape;
...
void GrainProcess::performMapping() {
  ...
  if(spread < 512) {
    int max_grains = 100;
    nGrains = (int) (max_grains * (1.0 - spread / 511.0) + 2);
  ...
  if(size < 800) {
    ...
    grainLen = (int) (divisionLen * (size / 511.0));
    std::cout << length << ", size: " << size / 511.0 << ", glen: " << grainLen << ", dlen: " << divisionLen << std::endl;
  } else if(size < 900) {
  ...
  if(shape < 511) {
    ...
    env_shape = 0;
    // set length
    env_length = 0.5 - (shape / 1023.0);
  } else {
    // Set exponential shape
    env_shape = 1;
    env_length = 0.5 - ((shape - 512) / 1023.0);
  }
}
